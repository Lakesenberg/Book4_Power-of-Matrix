...
class T800GetupEnvCfg(GetupEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        # Same spawn as official tracking. Do not tweak PhysX articulation flags here:
        # that path can hang inside sim.reset() on WSL CPU fallback.
        ...
        print("[getup] T800 spawn matches tracking (no articulation overrides)", flush=True)
